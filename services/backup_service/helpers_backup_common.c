...
static unsigned int backup_keep_running = 1;
static unsigned long int watched_items;

void backup_signal_handler(int signum)
{
	MSS_PRINT_DEBUG("<%s>", __func__);
	logger("SIGINT signal handled, the \"backup\" program is terminated!");
...
int helpers_get_backup_keep_running()
{
	return backup_keep_running;
}
...
static int read_events(queue_t q, int fd)
{
	...
	buffer_i = 0;
	while (buffer_i < r && backup_keep_running)
	{
...
static void handle_event(queue_entry_t event, thread_argument_t * arg)
{
	...
	else
	{
		...
		while (hi)
		{
			...
			{
				MSS_PRINT_INFO("Unable to find directory %s ! Exiting !", hi->value);
				return;
			}
		}
...
static int backup_check_fs_and_run(thread_argument_t * arg)
{
	...
	while (backup_keep_running && (NULL != (p = fts_read(ftsp))))
	{
		switch (p->fts_info)
		{
			case FTS_F:
